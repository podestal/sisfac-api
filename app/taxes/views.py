...
from .models import SunatRecord
...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    queryset = SunatRecord.objects.annotate(
        sort_priority=Case(
    ...
    #     """
    #     documents = SunatRecord.objects.filter(document_type='03').annotate(
    #         sort_priority=Case(
    #             When(sunat_issue_time__isnull=True, then=0),  # NULL = highest priority (newest)
    #             default=1,  # Has value = lower priority
    #             output_field=IntegerField()
    ...
    #     """
    #     documents = SunatRecord.objects.filter(document_type='01').annotate(
    #         sort_priority=Case(
    #             When(sunat_issue_time__isnull=True, then=0),  # NULL = highest priority (newest)
    #             default=1,  # Has value = lower priority
    #             output_field=IntegerField()
    ...
    #         from django.utils import timezone
    #         now = timezone.now()
    #         db_today_documents = SunatRecord.objects.filter(created_at__date=now.date())
    ...
    #                             document = SunatRecord.sync_from_sunat(target_document, processed_data)
    #                             if document:
    ...
    #         try:
    #             db_document = SunatRecord.objects.get(id=document_id)
    ...
    #         try:
    #             db_document = SunatRecord.objects.get(sunat_id=sunat_id)
    #             print(f"Found document in database: {db_document.serie}-{db_document.numero}")
    #         except SunatRecord.DoesNotExist:
    #             db_document = None
    ...
    #         try:
    #             updated_document = SunatRecord.objects.get(sunat_id=sunat_id)
    ...
    #         # Create document
    #         document = SunatRecord.objects.create(
